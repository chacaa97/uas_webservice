...
<form action="/checkout" method="post" class=" flex-1 pointer-events-auto">
    @csrf
    <input type="text" name="user_id" class="hidden" value="{{Auth::user()->id}}">
    <input type="text" name="book_id" class="hidden" value="{{$data_buku->id}}">
          <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                  ...
                  <ul role="list" class="-my-6 divide-y divide-gray-200">
                    <li class="flex py-6">
                      <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                        <img src="/buku/{{ $data_buku->gambar }}" alt="Salmon orange fabric pouch with match zipper, gray zipper pull, and adjustable hip belt." class="h-full w-full object-cover object-center">
                      </div>
                      ...
                      <div class="ml-4 flex flex-1 flex-col">
                        <div>
                          <div class="flex justify-between text-base font-medium text-gray-900">
                            <h3>
                              <a href="#">{{$data_buku->nama}}</a>
                            </h3>
                            <p class="ml-4">Rp. {{ number_format($data_buku->harga) }}</p>
                          </div>
                        </div>
            ...
            <div class="border-t border-gray-200 px-4 py-6 sm:px-6">
              <div class="flex justify-between text-base font-medium text-gray-900">
                <p>Subtotal</p>
                <p>Rp. {{ number_format($data_buku->harga) }}</p>
              </div>
              @if(isset($ongkir))
              <div class="flex justify-between text-base font-medium text-gray-900">
                <p>Ongkir</p>
                <p>Rp. {{ number_format($ongkir['costs'][0]['cost'][0]['value']) }}</p>
              </div>
              <div class="flex justify-between text-base font-medium text-gray-900">
                <p>Tujuan</p>
                <p>{{ $ongkirs['rajaongkir']['origin_details']['type'] }} {{$ongkirs['rajaongkir']['origin_details']['city_name']}} -ke- {{ $ongkirs['rajaongkir']['destination_details']['type'] }} {{ $ongkirs['rajaongkir']['destination_details']['city_name'] }}</p>
              </div>
              @endif
              <div class="flex justify-between text-base font-medium text-gray-900">
                <p>Total</p>
                @if(isset($ongkir))
                <input type="number" class="hidden" name="total_harga" value="{{ ($data_buku->harga) + ($ongkir['costs'][0]['cost'][0]['value']) }}">
                <input type="text" class="hidden" name="pengiriman" value=" {{ $ongkir['name'] }} - {{ $ongkir['costs'][0]['service'] }} - Rp. {{ number_format( $ongkir['costs'][0]['cost'][0]['value'] ) }} - {{ $ongkirs['rajaongkir']['origin_details']['type'] }} {{$ongkirs['rajaongkir']['origin_details']['city_name']}} -ke- {{ $ongkirs['rajaongkir']['destination_details']['type'] }} {{ $ongkirs['rajaongkir']['destination_details']['city_name'] }}">
                <p>Rp. {{ number_format(($data_buku->harga) + ($ongkir['costs'][0]['cost'][0]['value'])) }}</p>
                @else
                <input type="number" class="hidden" name="total_harga" value="{{ $data_buku->harga }}">
                <p>Rp. {{ number_format($data_buku->harga) }}</p>
                @endif
              </div>
              <div class="mt-6">
...
<section class="w-fit bg-white dark:bg-gray-900 rounded-lg">
  <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
      ...
      <form action="{{ route('cek-ongkir') }}" method="post">
        @csrf
        <input type="number" name="id" value="{{ $data_buku->id }}" class="hidden">
